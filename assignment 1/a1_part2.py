...
def crop_image(image_data: list, start_row: int, start_col: int, crop_height: int, crop_width: int) -> list:
    ...
    >>> crop_image(EXAMPLE_PIXEL_GRID,0,0,1,1)
    [[[128, 128, 128]]]
    """
    return [[list(image_data[row][col])for col in range(start_col, start_col + crop_width)]
            for row in range(start_row, start_row + crop_height)]
